...
class dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.stage == 1:
            ...
            if self.phase == 'test':
                ...
                if len(n_prop)==0: # 1++
                    proposals = p_prop[:, :3] # 1++
                else:
                    proposals = np.concatenate((p_prop[:, :3], n_prop), axis=0) # 原来
                ...
                return dict(
                    vid_name = vid_name, data = data.astype(np.float32), vid_label = vid_label, point_label = point_label, vid_duration = vid_duration,
    ...
    def process_label(self, vid_name, vid_len, sample_idx):
        ...
        if self.num_segments == -1:
            ...
            if self.args.cfg == 'activitynet':
                self.t_factor_point = temp_anno.shape[0] * 16 / vid_duration / (vid_fps * 16)
            for key in temp_df['point'].keys():
                ...
                if int(point * self.t_factor_point) >= len(temp_anno):
                    ...
                    # print(vid_name)
                    temp_anno[int(point * temp_anno.shape[0]*16/vid_duration/(vid_fps * 16))][class_idx] = 1
                    # temp_anno[len(temp_anno)-1][class_idx] = 1
            ...
            #--修改
            if sample_idx.shape[0] < self.num_segments:
                self.t_factor_point = sample_idx.shape[0] / (vid_fps * vid_duration)
                temp_anno = np.zeros([sample_idx.shape[0], self.num_class], dtype=np.float32)
            else:
                self.t_factor_point = self.num_segments / (vid_fps * vid_duration)
                temp_anno = np.zeros([self.num_segments, self.num_class], dtype=np.float32)
            #--
            ...
            for key in temp_df['point'].keys():
                ...
                # temp_anno[int(point * self.t_factor_point)][class_idx] = 1
                #--修改
                if int(point * self.t_factor_point) >= temp_anno.shape[0]:
                    temp_anno[-1][class_idx] = 1
                else:
                    temp_anno[int(point * self.t_factor_point)][class_idx] = 1
                #--
