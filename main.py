...
def main(args):
    ...
    if args.stage == 1:
        ...
        save_config(args, os.path.join(args.output_path_s2, "config.json"))
    # save_config(args, os.path.join(args.output_path_s1, "config.json"))
        ...
        if args.mode == 'train':
            ...
            for step in process_bar:
                ...
                if step % args.test_iter == 0:
                    ...
                    # print("\n Current test_acc:{:.4f} best_acc:{:.4f}".format(test_acc, best_acc))
                    # logger.log_value('acc/best acc', best_acc, step)
        else:
