    ...
    for subset in ['test', 'train']:
        ...
        for sample in tqdm(data_loader):
            ...
            if subset == 'train':
                ...
                segs = grouping(np.where(bkg_seed[0] > 0)[0])  # 如果没有背景帧，则下面的seg为空列表
                NP = []
                ...
                if len(segs) > 0:
                    while len(NP) < num_NP:
                        ...
                        elif len(seg) > 1: # 1++ 原来是 else:
                            start = int(random.uniform(seg[0], seg[-1]))
                            end = int(random.uniform(start, seg[-1]))
                            # print("start: {}, end: {}".format(start, end))

                            if len(seg) == 2: # 1++
                                end = int(random.uniform(start, seg[-1])+0.5)  # 在已选的片段里随机选择一个结束帧 # 1++
